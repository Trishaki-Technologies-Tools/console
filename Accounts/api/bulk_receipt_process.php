...
try {
    ...
    while (($row = fgetcsv($handle)) !== false) {
        ...
        // 2. Manage Invoice
        $invoiceNo = null;
        $invStmt = $conn->prepare("SELECT invoice_no, cumulative_total_paid FROM invoices WHERE client_id = ? ORDER BY id DESC LIMIT 1");
        $invStmt->bind_param("i", $clientId);
        $invStmt->execute();
        $invRes = $invStmt->get_result();
        
        if ($invRow = $invRes->fetch_assoc()) {
            // Invoice exists, link to it
            $invoiceNo = $invRow['invoice_no'];
            $cumulativeTotalPaidBefore = floatval($invRow['cumulative_total_paid']);
            $cumulativeTotalForThisRow = $cumulativeTotalPaidBefore + $paidNow;
            
            // Update invoice with new cumulative paid amount and status
            $newInvoiceStatus = ($cumulativeTotalForThisRow >= $totalCharged - 0.01) ? 'paid' : 'partially_paid';
            $updInv = $conn->prepare("UPDATE invoices SET cumulative_total_paid = ?, status = ? WHERE invoice_no = ?");
            $updInv->bind_param("dss", $cumulativeTotalForThisRow, $newInvoiceStatus, $invoiceNo);
            $updInv->execute();
        } else {
            ...
            $invoiceNo = ($paidNow >= $totalCharged - 0.01) ? generateInvoiceNumber($conn, $clientId, $type, null, $invoiceItemsJson, $rowYear) : null;
            ...
            $cumulativeTotalForThisRow = $paidNow;
        }

        // 3. Existing Receipts for /P logic
        $stmt = $conn->prepare("SELECT receipt_no FROM receipts WHERE client_id = ? ORDER BY id DESC LIMIT 1");
        $stmt->bind_param("i", $clientId);
        $stmt->execute();
        $continueFrom = ($res2 = $stmt->get_result()->fetch_assoc()) ? $res2['receipt_no'] : null;
        
        // 4. Prepare JSON for Receipt items
        $itemsArr = [[
            'description' => $desc,
            'paymentMode' => $mode,
            'date' => $date,
        ...
        // 5. Generate Receipt No and Insert
        $rowYear = function_exists('getFinancialYearYearFromDate') ? getFinancialYearYearFromDate($date) : date('Y', strtotime($date));
        $receiptNo = generateReceiptNumber($conn, $clientId, $type, $continueFrom, $itemsJson, $rowYear);
        ...
        // Insert into receipts table, linked to the invoice!
        ...
        $stmt->bind_param("sisssddss", $receiptNo, $clientId, $invoiceNo, $type, $itemsJson, $totalCharged, $cumulativeTotalForThisRow, $date, $status);
        $stmt->execute();
    ...
    $conn->commit();
    log_action($conn, 'CREATE', 'receipts', 0, "Bulk processed $count receipts via CSV");
