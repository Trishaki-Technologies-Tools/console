...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO expenses (description, amount, date, category_id, payment_mode_id, attachment, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("sdsiiss", $description, $amount, $date_db, $category_id, $payment_mode_id, $attachment_path, $created_at);
    
    if ($stmt->execute()) {
        $expense_id = $conn->insert_id;
        
        log_action($conn, 'ADD', 'expenses', $expense_id, "Added expense: $description (₹$amount)");
        
        // Log to transactions table
        $tStmt = $conn->prepare("INSERT INTO transactions (type, amount, date, reference_id, reference_table, description, created_at) VALUES ('expense', ?, ?, ?, 'expenses', ?, ?)");
        $tStmt->bind_param("dsiss", $amount, $date_db, $expense_id, $description, $created_at);
        $tStmt->execute();
