...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($oldResult && $oldResult->num_rows > 0) {
        ...
        if ($has_new_attachment) {
            $stmt = $conn->prepare("UPDATE incomes SET description = ?, amount = ?, date = ?, category_id = ?, payment_mode_id = ?, attachment = ?, created_at = ? WHERE id = ?");
            $stmt->bind_param("sdsiissi", $description, $amount, $date_db, $category_id, $payment_mode_id, $attachment_path, $created_at, $id);
        } else {
            $stmt = $conn->prepare("UPDATE incomes SET description = ?, amount = ?, date = ?, category_id = ?, payment_mode_id = ?, created_at = ? WHERE id = ?");
        ...
        if ($stmt->execute()) {
            ...
            $tStmt = $conn->prepare("UPDATE transactions SET amount = ?, date = ?, description = ?, created_at = ? WHERE reference_id = ? AND reference_table = 'incomes'");
            ...
            log_action($conn, 'EDIT', 'incomes', $id, "Edited income: $description (₹$amount)");
